...
public:
   ...
   bool              ReplaceFirst(const string substring, const string newstring);
protected:
...
bool CString::ReplaceFirst(const string substring,const string newstring)
  {
   bool   result=false;
   int    len,pos=-1;
   string tmp;
//---
   len=StringLen(substring);
   if((pos=StringFind(m_string,substring,pos))>=0)
     {
      tmp=StringSubstr(m_string,0,pos)+newstring;
      m_string=tmp+StringSubstr(m_string,pos+len);
      // to eliminate possible loops
